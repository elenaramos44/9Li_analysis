...
# ------------------------------------------------------------------------------
# Cálculo dinámico del rango contando los *_multilat.pkl generados en Stage 2
# ------------------------------------------------------------------------------
TOTAL_CHUNKS=0
...
if [[ "$EXTRA_FLAGS" == *"--bkg"* ]]; then
    ...
    for RUN in "${RUNS[@]}"; do
        COUNT=$(ls ${RESULTS_DIR}/run${RUN}/processed/Li9_clusters_chunk_*_BKG_multilat.pkl 2>/dev/null | wc -l)
        TOTAL_CHUNKS=$((TOTAL_CHUNKS + COUNT))
    done
else
    echo ">> STAGE 4 LAUNCHER (Gd): SIGNAL MODE DETECTED <<"
    for RUN in "${RUNS[@]}"; do
        COUNT=$(ls ${RESULTS_DIR}/run${RUN}/processed/Li9_clusters_chunk_*_multilat.pkl 2>/dev/null | grep -v BKG | wc -l)
        TOTAL_CHUNKS=$((TOTAL_CHUNKS + COUNT))
    done
fi

if [ "$TOTAL_CHUNKS" -eq 0 ]; then
    echo "Error: No multilateration .pkl files found for Gd Stage 4 in ${RESULTS_DIR}!"
    exit 1
fi
...
echo "--> Found ${TOTAL_CHUNKS} total chunks generated by Stage 2 for Gd"
echo "--> Submitting Stage 4 array with range: ${ARRAY_RANGE}"

# ------------------------------------------------------------------------------
# Lanzamiento del Slurm Array de Refinamiento para Gd
# ------------------------------------------------------------------------------
JOB_OUT=$(sbatch \
    --array=${ARRAY_RANGE} \
...
echo "Stage 4 (Gd) submitted successfully. Job ID: ${JOB_ID}"

# ------------------------------------------------------------------------------
# Encadenamiento con Stage 5 Launcher (Gd)
# ------------------------------------------------------------------------------
...
echo "==============================================================="
echo "Stage 4 and Stage 5 chain successfully submitted for Gd!"
echo "==============================================================="